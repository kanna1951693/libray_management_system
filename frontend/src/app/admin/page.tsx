...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<"members" | "add-book">("members");
  ...
  if (loading) {
    return (
      <div className="min-h-screen pt-28 flex items-center justify-center" style={{ background: bg }}>
        <div
        ...
        {activeTab === "members" ? (
                      ...
                      >
                        ...
                        <td className="px-5 py-4">
                          <span
                            ...
                            style={{
                              background: (STATUS_PILL[m.status] ?? STATUS_PILL.EXPIRED).bg,
                              color:      (STATUS_PILL[m.status] ?? STATUS_PILL.EXPIRED).color,
                            }}
                          >
                            {m.status}
                          </span>
          ...
          <div className="max-w-2xl">
            <div className="rounded-2xl p-6 sm:p-8" style={{ background: cardBg, border: `1px solid ${border}` }}>
              <h2 className="font-serif text-2xl font-bold mb-1" style={{ color: text }}>
                Add or Update Books
              </h2>
              ...
              <form onSubmit={handleAddBook} className="space-y-5">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input name="title" value={bookForm.title} onChange={handleBookChange} required
                      ...
                      className={inputClass} style={{ ...inputStyle, "--tw-ring-color": accent } as any} />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold mb-1.5" style={{ color: muted }}>Author *</label>
                    <input name="author" value={bookForm.author} onChange={handleBookChange} required
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input name="isbn" value={bookForm.isbn} onChange={handleBookChange} required
                      placeholder="e.g. 9780132350884"
                      className={inputClass} style={inputStyle} />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold mb-1.5" style={{ color: muted }}>Category *</label>
                    <select name="category" value={bookForm.category} onChange={handleBookChange} required
                      className={inputClass} style={inputStyle}>
                ...
                <div className="grid sm:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input name="publisher" value={bookForm.publisher} onChange={handleBookChange} required
                      placeholder="e.g. O'Reilly"
                      className={inputClass} style={inputStyle} />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold mb-1.5" style={{ color: muted }}>Year *</label>
                    <input name="year" type="number" min="1000" max={new Date().getFullYear() + 2}
                      value={bookForm.year} onChange={handleBookChange} required
                      className={inputClass} style={inputStyle} />
                  </div>
                  <div>
                    ...
                    <input name="totalCopies" type="number" min="1"
                      value={bookForm.totalCopies} onChange={handleBookChange} required
                      className={inputClass} style={inputStyle} />
                  </div>
                </div>
                ...
                <div className="grid sm:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input name="location" value={bookForm.location} onChange={handleBookChange}
                      placeholder="e.g. Shelf M-01"
                      className={inputClass} style={inputStyle} />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold mb-1.5" style={{ color: muted }}>Language</label>
                    <input name="language" value={bookForm.language} onChange={handleBookChange}
                      placeholder="English"
                      className={inputClass} style={inputStyle} />
                  </div>
                  <div>
                    ...
                    <input name="pages" type="number" min="1"
                      value={bookForm.pages} onChange={handleBookChange}
                      placeholder="e.g. 450"
                      className={inputClass} style={inputStyle} />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input name="coverImage" value={bookForm.coverImage} onChange={handleBookChange}
                    placeholder="https://covers.openlibrary.org/b/isbn/9780...L.jpg"
                    className={inputClass} style={inputStyle} />
                  {bookForm.coverImage && (
                ...
                <div>
                  ...
                  <textarea name="description" value={bookForm.description} onChange={handleBookChange}
                    placeholder="A brief overview of the book…" rows={3}
                ...
                {bookSuccess && (
                  <div className="rounded-xl px-4 py-3 text-sm font-medium"
                    style={{ background: "rgba(34,197,94,0.10)", color: isDark ? "#22C55E" : "#166534", border: "1px solid rgba(34,197,94,0.25)" }}>
                    {bookSuccess}
                  </div>
                )}
                {bookError && (
                  <div className="rounded-xl px-4 py-3 text-sm font-medium"
                    style={{ background: "rgba(239,68,68,0.10)", color: isDark ? "#EF4444" : "#B91C1C", border: "1px solid rgba(239,68,68,0.25)" }}>
                    {bookError}
                  </div>
                )}
